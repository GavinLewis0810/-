...
class FieldExtractor:
    ...
    PATTERNS_CN = {
        ...
        # 新增字段：规格型号、单位、数量、单价
        'specification': r'规格型号\s+([^单位]+?)(?=\s+单位|\n|$)',
        'unit': r'单位\s+([^\s\d]+)',
        'quantity': r'数量\s+(\d+(?:\.\d+)?)',
        'unit_price': r'单价\s+(\d+(?:\.\d{2})?)',
    }
    ...
    def extract_fields(self, text: str, lines: Optional[List[Dict[str, Any]]] = None) -> Tuple[Dict[str, Any], Dict[str, Optional[float]]]:
        ...
        # ========== 新增：金额字段的备用提取（不破坏原有逻辑）==========
        # 只有当原有金额为空或无效时，才尝试备用提取
        if not self._is_valid_amount(fields.get('amount')):
    ...
    def _clean_value(self, value: str, field_name: str) -> Any:
        ...
        if field_name in ['buyer_name', 'seller_name', 'item_name', 'specification']:
...
#新加用法************
    ...
    def _extract_amount_from_invoice(self, text: str) -> Optional[str]:
        """备用提取金额：优先寻找不含税合计金额"""
        # 1. 匹配“计”字后面紧跟的金额（常见于电子发票）
        match = re.search(r'计\s*[¥￥]?\s*([\d,]+\.?\d{0,2})', text)
        if match:
            return match.group(1).replace(',', '')
        # 2. 匹配“合计”字样（含完整“合计”）
        match = re.search(r'合计\s*[¥￥]?\s*([\d,]+\.?\d{0,2})', text)
        if match:
            return match.group(1).replace(',', '')
        # 3. 匹配“金额(元)”列下的第一个数值（原备用方案）
        lines = text.splitlines()
        for i, line in enumerate(lines):
            if '金额' in line and '(元)' in line:
                for j in range(i + 1, min(i + 5, len(lines))):
                    num_match = re.search(r'(\d+\.?\d{0,2})', lines[j])
                    if num_match and '税额' not in lines[j] and '税率' not in lines[j]:
                        return num_match.group(1)
        return None

    def _extract_tax_rate_from_invoice(self, text: str) -> Optional[str]:
        """备用提取税率"""
        # 匹配“税率”后面的数字+%
        match = re.search(r'税率\s*(\d+(?:\.\d+)?%|免税)', text)
        if match:
...
#新增：当原有的税额值缺少小数部分（例如 665. 或 665），则尝试从原始文本中提取完整的带两位小数的金额。
    def _refine_tax_amount(self, text: str, original_value: Optional[str]) -> Optional[str]:
        """如果原有税额缺少小数部分，尝试从文本中提取完整的两位小数税额"""
        if not original_value:
            return original_value

        # 如果已经有小数点且后面有至少两位数字，直接返回
        if '.' in original_value and len(original_value.split('.')[1]) >= 2:
            return original_value

        # 提取原有值的整数部分（例如 "665." -> "665"）
        import re
        int_part = re.sub(r'\..*$', '', original_value) if '.' in original_value else original_value
        int_part = int_part.strip()

        # 1) 优先匹配“税额”标签后的数字（允许空格）
        match = re.search(r'税\s*额\s*[¥￥]?\s*(\d+\.\s*\d{2})', text)
        if match:
            raw = match.group(1)
            cleaned = re.sub(r'(\d)\.\s+(\d)', r'\1.\2', raw)
            cleaned = re.sub(r'\s+', '', cleaned)
            return cleaned

        # 2) 根据整数部分查找对应的完整金额（例如查找 "665.42" 或 "665. 42"）
        # 匹配模式：整数部分 + 点 + 可选空格 + 两位数字
        pattern = rf'{re.escape(int_part)}\.\s*(\d{{2}})'
        match = re.search(pattern, text)
        if match:
            # 重组完整金额
            decimal_part = match.group(1)
            full_amount = f"{int_part}.{decimal_part}"
            return full_amount

        # 3) 备用：查找所有两位小数的金额，选择数值最小的（税额通常最小）
        all_amounts = re.findall(r'(\d+\.\d{2})', text)
        if all_amounts:
            # 转换为浮点数比较（注意精度，这里只是比较大小）
            min_amount = min(all_amounts, key=lambda x: float(x))
            # 确保该金额与原有整数部分相近（可选）
            if min_amount.startswith(int_part) or float(min_amount) < float(original_value or 0) * 2:
                return min_amount
