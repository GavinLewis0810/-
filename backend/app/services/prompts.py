...
def build_ai_check_prompt(reimb_title: str, reimb_amount: str, invoices: list) -> str:
    """
...
"""
    ...
    prompt = f"""请对以下报销单进行严格合规审计，检查是否存在逻辑冲突或异常。
...
## 审计要点
1. 报销事由与发票商品明细是否匹配（例如：报销事由是"出差差旅"，但发票里买了"键盘鼠标"，则判定不合规）
2. 金额是否合理
3. 是否存在可疑项目

## 输出要求
请严格按照以下 JSON 格式输出审计结果（只输出 JSON，不要多余文字）：
{{
